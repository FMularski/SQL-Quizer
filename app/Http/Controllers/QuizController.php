...
class QuizController extends Controller
{
    ...
    public function sqlserver()
    {
        ...
        $sqlserver_tasks_ids_all = DB::select('SELECT Id FROM Tasks WHERE Category = ?', ['sqlserver']);
        $sqlserver_tasks_answers_all = DB::select('SELECT Answer FROM Tasks WHERE Category = ?', ['sqlserver']);
        ...
        for ( $i = 0; $i < count($indexes); $i++)
        {
            array_push($ids, $sqlserver_tasks_ids_all[$indexes[$i]]->Id);
            ...
            array_push($correct, $sqlserver_tasks_answers_all[$indexes[$i]]->Answer);
        }

        return view('sqlserver', ['ids' => $ids, 'texts' => $texts, 'paths' => $paths, 'correct' => $correct]);
    }
    ...
    public function oracle()
    {
        ...
        $oracle_tasks_ids_all = DB::select('SELECT Id FROM Tasks WHERE Category = ?', ['oracle']);
        $oracle_tasks_answers_all = DB::select('SELECT Answer FROM Tasks WHERE Category = ?', ['oracle']);
        ...
        for ( $i = 0; $i < count($indexes); $i++)
        {
            array_push($ids, $oracle_tasks_ids_all[$indexes[$i]]->Id);
            array_push($texts, $oracle_tasks_texts_all[$indexes[$i]]->Text);
            array_push($paths, $oracle_tasks_img_paths_all[$indexes[$i]]->ImgPath);
            array_push($correct, $oracle_tasks_answers_all[$indexes[$i]]->Answer);
        }

        return view('oracle', ['ids' => $ids, 'texts' => $texts, 'paths' => $paths, 'correct' => $correct]);
    }
    ...
    public function postgresql()
    {
        ...
        $postgresql_tasks_ids_all = DB::select('SELECT Id FROM Tasks WHERE Category = ?', ['postgresql']);
        $postgresql_tasks_answers_all = DB::select('SELECT Answer FROM Tasks WHERE Category = ?', ['postgresql']);
        ...
        for ( $i = 0; $i < count($indexes); $i++)
        {
            array_push($ids, $postgresql_tasks_ids_all[$indexes[$i]]->Id);
            ...
            array_push($correct, $postgresql_tasks_answers_all[$indexes[$i]]->Answer);
        }

        return view('postgresql', ['ids' => $ids, 'texts' => $texts, 'paths' => $paths, 'correct' => $correct]);
    }
    ...
    public function check(Request $request)
    {
        ...
        for ( $i = 0; $i < 5; $i++)
        {
            if ( strcmp( strtolower(str_replace(array(PHP_EOL, ' '), '', $answers[$i])), 
                        strtolower(str_replace(array(PHP_EOL, ' '), '', $corrects[$i]))) == 0)
            {
                $points++;
                $correct_flags[$i] = 1;
            } 
